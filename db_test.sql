...
SELECT m.id, m.name, m.time_create, p.status AS pendidikan_terakhir, p.time_create AS time_update FROM murid m LEFT JOIN pendidikan p
ON m.id = p.id_murid
WHERE p.time_create = (
